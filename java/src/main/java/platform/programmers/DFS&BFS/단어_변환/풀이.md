...
아래 다이어그램은 단어 변환 BFS 탐색의 전체 흐름을 시각화한 것입니다.

```mermaid
flowchart TD
    Start["시작: begin 단어를 큐에 삽입\nstep = 0"] --> TargetCheck{"target이\nwords에 존재하는가?"}
    TargetCheck -- "No" --> ReturnZero["0 반환"]
    TargetCheck -- "Yes" --> Loop{"큐가 비어있지 않은가?"}
    Loop -- "Yes" --> Poll["큐에서 (word, step) 꺼냄"]
    Poll --> GoalCheck{"word == target?"}
    GoalCheck -- "Yes" --> ReturnStep["step 반환 (최소 변환 수)"]
    GoalCheck -- "No" --> WordLoop["words 배열 순회\ni = 0 ~ words.length-1"]
    WordLoop --> VisitCheck{"visited[i] == false?"}
    VisitCheck -- "No" --> NextWord["다음 단어"]
    VisitCheck -- "Yes" --> ConvertCheck{"canConvert(word, words[i])?\n(한 글자만 다른가?)"}
    ConvertCheck -- "No" --> NextWord
    ConvertCheck -- "Yes" --> Enqueue["visited[i] = true\n큐에 (words[i], step+1) 삽입"]
    Enqueue --> NextWord
    NextWord --> WordLoop
    WordLoop -- "순회 완료" --> Loop
    Loop -- "No (큐 비어있음)" --> ReturnZeroEnd["0 반환 (변환 불가)"]
```
...
bool canConvert(const string& w1, const string& w2) {
    int diff = 0;
...
int solution(string begin, string target, vector<string> words) {
    ...
    for (const auto& w : words) {
        if (w == target) { found = true; break; }
    }
    if (!found) return 0;

    // BFS 탐색
    vector<bool> visited(words.size(), false);
    queue<pair<string, int>> q; // {단어, 단계수}
    q.push({begin, 0});
...
// 두 단어가 한 글자만 다른지 확인하는 함수
fn can_convert(w1: &str, w2: &str) -> bool {
    let diff = w1.chars().zip(w2.chars())
        .filter(|(a, b)| a != b)
        .count();
    diff == 1
}

pub fn solution(begin: &str, target: &str, words: Vec<String>) -> i32 {
    // target이 words에 없으면 변환 불가
    ...
    queue.push_back((begin.to_string(), 0)); // (단어, 단계수)
    ...
    while let Some((word, step)) = queue.pop_front() {
        ...
        for i in 0..words.len() {
            if !visited[i] && can_convert(&word, &words[i]) {
                visited[i] = true;
                queue.push_back((words[i].clone(), step + 1));
            }
        }
    }
...
// canConvert는 두 단어가 한 글자만 다른지 확인한다.
func canConvert(w1, w2 string) bool {
    diff := 0
    for i := 0; i < len(w1); i++ {
        if w1[i] != w2[i] {
            diff++
...
func solution(begin, target string, words []string) int {
    // target이 words에 없으면 변환 불가
    found := false
    for _, w := range words {
        if w == target {
    ...
    for len(queue) > 0 {
        ...
        for i, w := range words {
            if !visited[i] && canConvert(cur.word, w) {
                visited[i] = true
                queue = append(queue, State{w, cur.step + 1})
            }
        }
    }
...
| 관점 | 케이스 | 처리 방법 |
|---|---|---|
| target 부재 | target이 words 배열에 없는 경우 | 탐색 전에 미리 체크하여 0 반환 |
| 직접 변환 | begin에서 target으로 한 글자만 바꾸면 되는 경우 | BFS 1단계에서 바로 발견, step=1 반환 |
| 변환 경로 없음 | words에 target은 있지만 경로가 연결 안 되는 경우 | 큐가 비어도 target 미도달 시 0 반환 |
| 동일 단어 중복 | words에 같은 단어가 여러 번 등장 | visited 배열이 인덱스 기반이므로 중복 방문 방지 |
| 긴 단어 | 단어 길이가 큰 경우 (최대 10) | canConvert에서 O(L) 비교, 전체 복잡도에 미미한 영향 |
...
| 풀이 | 시간 복잡도 | 공간 복잡도 | 비고 |
|---|---|---|---|
| BFS | O(N^2 * L) | O(N) | N=단어 수, L=단어 길이. 각 단어마다 words 전체와 비교 |
| DFS (대안) | O(N! * L) 최악 | O(N) | 모든 순열 탐색 필요, 비효율적 |
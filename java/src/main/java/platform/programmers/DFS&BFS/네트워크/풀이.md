...
아래 다이어그램은 DFS 기반 네트워크 탐색의 전체 흐름을 시각화한 것입니다.

```mermaid
flowchart TD
    Start["시작: i = 0 ~ n-1 순회"] --> CheckVisit{"visit[i] == false?"}
    CheckVisit -- "Yes" --> NewNetwork["새 네트워크 발견\nanswer++"]
    NewNetwork --> DFS["dfs(i) 호출"]
    DFS --> MarkVisit["visit[idx] = true"]
    MarkVisit --> ScanLoop["j = 0 ~ n-1 순회"]
    ScanLoop --> CheckConnect{"idx != j AND\n!visit[j] AND\ncomputers[idx][j] == 1?"}
    CheckConnect -- "Yes" --> RecurseDFS["dfs(j) 재귀 호출"]
    RecurseDFS --> MarkVisit
    CheckConnect -- "No" --> NextJ["j++"]
    NextJ --> ScanLoop
    ScanLoop -- "j 순회 완료" --> ReturnDFS["dfs 종료, 호출자로 복귀"]
    ReturnDFS --> CheckVisit
    CheckVisit -- "No (이미 방문)" --> NextI["i++"]
    NextI --> CheckVisit
    CheckVisit -- "i 순회 완료" --> Result["answer 반환"]
```
...
function solution(n, computers) {
    ...
    // DFS로 연결된 모든 컴퓨터를 방문 처리
    function dfs(idx) {
        visit[idx] = true;
        for (let i = 0; i < n; i++) {
            if (idx !== i && !visit[i] && computers[idx][i] === 1) {
                dfs(i);
            }
        }
    }

    // 모든 컴퓨터를 순회하며 새 네트워크 탐색
    for (let i = 0; i < n; i++) {
        if (!visit[i]) {
            dfs(i);
            answer++;
...
```rust
pub fn solution(n: usize, computers: Vec<Vec<i32>>) -> i32 {
    let mut visit = vec![false; n];
    let mut answer = 0;

    // DFS로 연결된 모든 노드 방문
    fn dfs(idx: usize, n: usize, computers: &Vec<Vec<i32>>, visit: &mut Vec<bool>) {
        visit[idx] = true;
        for i in 0..n {
            if idx != i && !visit[i] && computers[idx][i] == 1 {
                dfs(i, n, computers, visit);
    ...
    // 미방문 노드를 발견할 때마다 새 네트워크 카운트
    for i in 0..n {
        if !visit[i] {
            dfs(i, n, &computers, &mut visit);
            answer += 1;
        }
    }
...
// dfs는 idx 노드에서 출발하여 연결된 모든 노드를 방문 처리한다.
func dfs(idx, n int, computers [][]int, visit []bool) {
    visit[idx] = true
    for i := 0; i < n; i++ {
        // 자기 자신이 아니고, 미방문이며, 연결되어 있으면 재귀
        if idx != i && !visit[i] && computers[idx][i] == 1 {
            dfs(i, n, computers, visit)
        }
    }
...
| 관점 | 케이스 | 처리 방법 |
|---|---|---|
| 최소 입력 | n=1, 컴퓨터가 1대뿐인 경우 | 자기 자신만 있으므로 네트워크 수는 1 |
| 완전 연결 | 모든 컴퓨터가 서로 연결된 경우 | DFS 한 번에 전부 방문되어 answer=1 |
| 완전 분리 | 어떤 컴퓨터도 서로 연결되지 않은 경우 | 각 컴퓨터가 독립 네트워크, answer=n |
| 자기 자신 연결 | computers[i][i] = 1 (대각선) | idx != i 조건으로 자기 자신 재귀 방지 |
| 간접 연결 | A-B 연결, B-C 연결이지만 A-C는 미연결 | DFS가 A->B->C 순서로 모두 방문하여 하나의 네트워크로 인식 |
...
| 풀이 | 시간 복잡도 | 공간 복잡도 | 비고 |
|---|---|---|---|
| DFS (재귀) | O(N^2) | O(N) | visit 배열 + 재귀 스택 |
| BFS (큐) | O(N^2) | O(N) | visit 배열 + 큐 |
| OOP 스타일 | O(N^2) | O(N^2) | 인접 리스트 저장 공간 추가 |

...
아래 다이어그램은 BFS를 이용한 게임 맵 최단거리 탐색의 흐름을 시각화한 것입니다.

```mermaid
flowchart TD
    Start["시작: (0,0)을 큐에 삽입\nvisited[0][0] = true\ndist = 1"] --> Loop{"큐가 비어있지 않은가?"}
    Loop -- "Yes" --> Poll["큐에서 (cx, cy, dist) 꺼냄"]
    Poll --> GoalCheck{"cx == n-1 AND\ncy == m-1?"}
    GoalCheck -- "Yes" --> ReturnDist["dist 반환 (최단거리)"]
    GoalCheck -- "No" --> DirLoop["4방향(상하좌우) 탐색\ni = 0 ~ 3"]
    DirLoop --> BoundCheck{"(nx, ny)가\n맵 범위 내인가?"}
    BoundCheck -- "No" --> NextDir["다음 방향"]
    BoundCheck -- "Yes" --> WallCheck{"maps[nx][ny] == 1\nAND !visited[nx][ny]?"}
    WallCheck -- "No" --> NextDir
    WallCheck -- "Yes" --> Enqueue["visited[nx][ny] = true\n큐에 (nx, ny, dist+1) 삽입"]
    Enqueue --> NextDir
    NextDir --> DirLoop
    DirLoop -- "4방향 완료" --> Loop
    Loop -- "No (큐 비어있음)" --> ReturnFail["-1 반환 (도달 불가)"]
```
...
pub fn solution(maps: Vec<Vec<i32>>) -> i32 {
    let n = maps.len();
    let m = maps[0].len();
    ...
    queue.push_back((0i32, 0i32, 1));
    visited[0][0] = true;

    while let Some((cx, cy, dist)) = queue.pop_front() {
        // 도착점 도달 시 거리 반환
        if cx == (n as i32 - 1) && cy == (m as i32 - 1) {
            return dist;
        }
...
| 관점 | 케이스 | 처리 방법 |
|---|---|---|
| 도달 불가 | 벽으로 막혀 목적지까지 갈 수 없는 경우 | 큐가 빌 때까지 도달 못하면 -1 반환 |
| 최소 맵 | 1x1 맵 (시작=도착) | 시작점이 곧 도착점이므로 거리 1 반환 |
| 일직선 경로 | 벽 없이 오른쪽+아래로만 가는 경우 | BFS가 최단거리 (n+m-1) 정확히 계산 |
| 좁은 통로 | 1칸 폭의 미로 형태 | BFS가 유일한 경로를 따라가며 거리 계산 |
| 큰 맵 | n=100, m=100 | BFS는 O(N*M)이므로 최대 10,000칸 처리 가능 |
...
| 풀이 | 시간 복잡도 | 공간 복잡도 | 비고 |
|---|---|---|---|
| BFS (권장) | O(N * M) | O(N * M) | 모든 칸을 최대 한 번 방문, visited 배열 |
| DFS (비추천) | O(4^(N*M)) 최악 | O(N * M) | 모든 경로를 탐색해야 하므로 비효율적 |
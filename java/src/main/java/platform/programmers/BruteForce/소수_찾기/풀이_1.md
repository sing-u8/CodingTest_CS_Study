...
fn solution(numbers: &str) -> i32 {
    ...
    let chars: Vec<char> = numbers.chars().collect();
    ...
    fn generate(
        ...
        for i in 0..chars.len() {
            if !used[i] {
                used[i] = true;
                generate(chars, used, current.clone() + &chars[i].to_string(), set);
                used[i] = false; // 백트래킹
            }
        }
    }

    generate(&chars, &mut used, String::new(), &mut number_set);

    // 소수 판별 후 개수 반환
    number_set.iter().filter(|&&n| is_prime(n)).count() as i32
}
...
func solution(numbers string) int {
	...
	generate = func(current string) {
		...
		for i := 0; i < len(chars); i++ {
			if !used[i] {
				used[i] = true
				generate(current + string(chars[i]))
				used[i] = false // 백트래킹
			}
		}
	}
...
```mermaid
flowchart TD
    A[시작: numbers 입력] --> B[재귀 함수 호출: recursive 빈문자열, numbers]
    B --> C{현재 조합이 비어있지 않은가?}
    C -->|Yes| D[Set에 숫자 추가]
    C -->|No| E[남은 문자 순회]
    D --> E
    E --> F{남은 문자가 있는가?}
    F -->|Yes| G[한 문자 선택하여 조합에 추가]
    G --> H[선택한 문자 제외한 나머지로 재귀 호출]
    H --> C
    F -->|No| I[리턴 - 백트래킹]
    I --> J{모든 순열 생성 완료?}
    J -->|No| E
    J -->|Yes| K[Set의 각 숫자에 대해 소수 판별]
    K --> L{num < 2?}
    L -->|Yes| M[소수 아님]
    L -->|No| N{2부터 sqrt N 까지 나누어 떨어지는가?}
    N -->|Yes| M
    N -->|No| O[소수 - count 증가]
    O --> P[count 반환]
```
...
| 관점 | 케이스 | 처리 방법 |
|---|---|---|
| 중복 숫자 | numbers = "011" | Set 사용으로 "11"과 "11" 중복 자동 제거, Integer.valueOf로 앞의 0 제거 |
| 한 자리 숫자 | numbers = "7" | 재귀에서 "7" 하나만 생성, 소수 판별 후 1 반환 |
| 0과 1만 있는 경우 | numbers = "011" | 0과 1은 소수가 아니므로 11만 소수로 판별 |
| 모두 같은 숫자 | numbers = "1111" | Set으로 중복 제거 → {1, 11, 111, 1111}, 모두 소수 아님 → 0 반환 |
| 소수가 없는 경우 | numbers = "0" | 0은 소수가 아니므로 0 반환 |
...
| 풀이 | 시간 복잡도 | 공간 복잡도 | 비고 |
|---|---|---|---|
| 재귀 순열 + 소수 판별 | O(N! × N + K × √M) | O(N!) | N=숫자 개수(최대 7), K=생성된 숫자 수, M=최대 숫자값. 순열 생성 O(N!), 소수 판별 O(√M) |
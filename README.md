...
프로그래머스/LeetCode 코딩 테스트 및 SQL 풀이를 **한 저장소**에서 정리하기 위한 템플릿입니다.

## 구성

- `java/`: Java 17(Gradle) 풀이/유틸
- `js/`: JavaScript(기본) 풀이/유틸 + (옵션) TypeScript 지원
- `cpp/`: C++17(Makefile) 풀이/유틸
- `go/`: Go(Makefile) 풀이/유틸
- `rust/`: Rust(Cargo) 풀이/유틸
- `sql/`: SQL 문제 풀이 + 스니펫
...
- **플랫폼별 풀이 경로**
  ...
  - Rust: `rust/src/bin/` (평탄 구조 — 파일명 prefix로 플랫폼 구분)
- **파일명**
  - LeetCode: `LC_<문제번호>_<slug>.java|js|ts|cpp|rs|go`
  - 프로그래머스: `P_<레벨>_<문제명>.java|js|sql|cpp|rs|go`
- **노트(MD)**
...
- 특정 파일 실행:
  - `node src/platform/leetcode/LC_0000_template.js`
- 러너로 실행(디버그 편의):
